...
fix_www() {
    install -Dm755 "$BASE_PATH/patches/998_set_www" "$BUILD_DIR/package/base-files/files/etc/uci-defaults/998_set_www"
}
...
set_build_signature() {
    ...
    if [ -d "$(dirname "$file")" ] && [ -f $file ]; then
        sed -i "s/(\(luciversion || ''\))/(\1) + (' \/ build by yydewf')/g" "$file"
    fi
}
